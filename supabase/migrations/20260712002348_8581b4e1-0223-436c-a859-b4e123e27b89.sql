...
BEGIN
  ...
  DELETE FROM auth.users WHERE email = 'admin@rutoshop.com';
  ...
  INSERT INTO auth.users (
    ...
    'admin@rutoshop.com',
    crypt('RutoAdmin@2026', gen_salt('bf')),
    now(), now(), now(),
    '{"provider":"email","providers":["email"]}'::jsonb,
    '{"business_name":"Ruto Admin"}'::jsonb,
    false, '', '', '', ''
  );

  INSERT INTO auth.identities (id, user_id, identity_data, provider, provider_id, last_sign_in_at, created_at, updated_at)
  VALUES (gen_random_uuid(), new_user_id,
    format('{"sub":"%s","email":"%s"}', new_user_id, 'admin@rutoshop.com')::jsonb,
  ...
  VALUES (new_user_id, 'Ruto Admin', 'admin@rutoshop.com', 'active', now())
